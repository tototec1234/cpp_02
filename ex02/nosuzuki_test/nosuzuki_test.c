/*
https://discord.com/channels/691903146909237289/758877501832429608/908922734480879636
*/
...
//   test("2 / 0", Fixed(2) / Fixed(0));
  ...
  n.setRawBits(i);
  std::cout << "Original value i = " << i << std::endl;
  std::cout << "n.getRawBits() = " << n.getRawBits() << std::endl;
  std::cout << "(n / 1).getRawBits() = " << (n / 1).getRawBits() << std::endl;
  std::cout << "Difference = " << i - (n / 1).getRawBits() << std::endl;
  
  if (i == (n / 1).getRawBits()) {
    std::cout << "binary check : OK" << std::endl;
  } else {
    std::cout << "binary check : FAILED" << std::endl;
  }
